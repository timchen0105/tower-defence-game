...
#include "Turret/GrenadeTurret.hpp"
		...
		if (mapState[y][x] != TILE_OCCUPIED) {
			...
			if (toolPreview) {
				for(auto it : TowerGroup->GetObjects()){
					if (Engine::Collider::IsPointInRect(Engine::Point(mx, my), it->Position, Engine::Point(BlockSize, BlockSize))) {
...
void PlayScene::UIBtnClicked(int id) {
	...
	else if (id == 3 && money >= GrenadeTurret::Price)
		preview = new GrenadeTurret(0, 0);
	else if (id == 4)
		toolPreview = new Shovel(0, 0);
	if (preview) {
		preview->Position = Engine::GameEngine::GetInstance().GetMousePosition();
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
	...
	while (!que.empty()) {
        ...
        for(int i = 0; i < 4; i++){
			next_x = p.x + dx[i];
			next_y = p.y + dy[i];
			if(next_y >= 0 && next_y < MapHeight && next_x >= 0 && next_x < MapWidth){
